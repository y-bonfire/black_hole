...
public:
    ...
    Ray(double x, double y, double vx, double vy, double ax = 0.0, double ay = 0.0)
        : position{x, y}, velocity{vx, vy}, acceleration{ax, ay}
    {
    }
};

Ray ray1(-1e11f, 0.0f, 2e11f / 10.0f, 0.0f, 5e10f, 0.0f);
Ray ray2(-1e11f, -5e10f, 2e11f / 10.0f, 0.0f, 0.0f, 0.0f);
Ray ray3(-1e11f, 5e10f, 2e11f / 10.0f, 0.0f, 0.0f, 0.0f);
std::vector<Ray> rays{ray1, ray2, ray3};
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        update();
        if (currentTime - lastTime >= .5f && currentTime < 5.0f)
        {
            ray1 = rays[0];
            std::cout << "Ray 1 Cartesian Coordinates: (x: " << ray1.position.x << ", y: " << ray1.position.y << ")\n";
            ray1.convert_to_cartesian();
            std::cout << "Ray 1 Polar Coordinates: (r: " << ray1.r << ", phi: " << ray1.phi << ")\n";
            std::cout << "Ray 1 Cartesian Coordinates: (x: " << ray1.position.x << ", y: " << ray1.position.y << ")\n";
        }
